...
def make_d_block(sweep, outFile, dumpfile, verbose=False):
    ...
    physLen = 500000
    subPopSize = 10000 / 5 #Have to scale sampling times back too 
...
def run_slim(d_block_burn, d_block_sel, logfile):
    ...
    cmd1 = " ".join(
        ["time slim", d_block_burn, "constant_burn.slim", ">>", logfile]
    ).replace("    ", "")
    cmd2 = " ".join(
        ["time slim", d_block_sel, "constant_selection.slim", ">>", logfile]
    ).replace("    ", "")
